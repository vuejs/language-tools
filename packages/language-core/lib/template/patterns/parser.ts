// Mirrored from vuejs/core packages/compiler-core/src/patterns.ts for RFC 823.
// This Draft can be tested against released compilers; use the shared export
// when the compiler reference implementation becomes an available dependency.
...
export function parseMatchPattern(source: string): MatchArm {
	...
	const word = (text: string) => {
		space();
		if (!source.startsWith(text, pos) || /^[$_\u200c\u200d\p{ID_Continue}]/u.test(source.slice(pos + text.length))) {
			return false;
		}
		pos += text.length;
		return true;
	...
	const bind = (): PatternBinding => {
		const result = name();
		if (reserved.has(result.name)) {
			fail(`Invalid binding name ${result.name}.`);
		}
	...
	const string = () => {
		...
		while (pos < source.length) {
			let char = source[pos++];
			if (char === quote) {
				return { value, text: source.slice(start, pos) };
			}
			if (char === '\n' || char === '\r') {
			...
			if (char === '\\') {
				...
				else if (/[1-9]/.test(char || '') || (char === '0' && /\d/.test(source[pos] || ''))) {
	...
	const rest = (close: string): PatternRest => {
		...
		space();
		if (source[pos] !== close) {
			fail('Rest must be last, without a trailing comma; use ... or ...const name.');
		}
	...
	const parse = (): MatchPattern => {
		...
		if (alternatives.length > 1) {
			...
			pattern = { kind: 'or', patterns: alternatives, start: pattern.start, end: pos };
		}
		if (word('as')) {
			const binding = bind();
	...
	const atom = (): MatchPattern => {
		...
		if (eat('{')) {
			...
			while (!eat('}')) {
				...
				if (keys.has(key)) {
					fail(`Duplicate pattern property ${key}.`);
				}
				keys.add(key);
		...
		if (sign) {
			fail('Expected a numeric literal after sign.');
		}
		const value = name();
		if (value.name === '_') {
			return { kind: 'wildcard', start, end: pos };
		}
		...
		return { kind: 'value', text: source.slice(start, pos).trim(), start, end: pos };
	};
	const pattern = parse();
	...
	if (pos !== source.length) {
		fail('Unexpected token in pattern.');
	}
