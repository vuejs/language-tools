...
/** Runs a content mapper process: reads protocol messages from stdin and writes results to stdout. */
export function runMapperServer(mapper: ContentMapper, options: { workerPath: string }) {
	let input = Buffer.alloc(0);
	const workerPath = readArgument('worker') ?? options.workerPath;
	const typescriptPath = readArgument('typescript');
	if (typescriptPath) {
		process.env.VUE_CONTENT_MAPPER_TYPESCRIPT = typescriptPath;
	}
	const transformPool = new TransformPool(mapper, undefined, workerPath);
	...
	function initialize(params: InitializeParams): InitializeResult {
		...
		return {
			...
			diagnosticSource: mapper.diagnosticSource,
		};
	}
	...
	function selectPositionEncoding(encodings: PositionEncoding[]): PositionEncoding {
		...
		throw new Error(`${mapper.name} requires UTF-16 position encoding`);
	}
...
/** Runs a content mapper worker thread, which owns the project state of the worker pool. */
export function runMapperWorker(mapper: ContentMapper) {
	if (!parentPort) {
		throw new Error(`${mapper.name} worker requires a parent port`);
	}

	parentPort.on('message', ({ id, method, params }) => {
		try {
			const result = method === 'openProject'
				? mapper.openProject(params as OpenProjectParams)
				: method === 'closeProject'
				? mapper.closeProject((params as CloseProjectParams).projectHandle)
				: mapper.transform(params as TransformParams);
