...
function resolvePlugins(
  ...
  plugins.typescript = (_context): LanguageServicePluginInstance => {
    const base =
      typeof originalTsPlugin === "function"
        ? originalTsPlugin(_context)
        : originalTsPlugin;

    if (!_context?.typescript) return base;
    ...
    return {
      ...
      resolveEmbeddedRange(range) {
        if (
          autoImportPositions.has(range.start) &&
          autoImportPositions.has(range.end)
        )
          return range;
      },
      async provideCompletionItems(document, position, context, item) {
        const result = await base.provideCompletionItems?.(
          document,
          position,
          context,
          item
          ...
          result.items = result.items.filter(
            (item) =>
              ...
              (!item.labelDetails?.description ||
                item.labelDetails.description.indexOf("__VLS_") === -1)
          );

          // handle component auto-import patch
          let sourceIsKebabCasing: boolean | null = null;
          for (const [_, map] of _context.documents.getMapsByVirtualFileUri(
            document.uri
          )) {
            const virtualFile = _context.documents.getSourceByUri(
              map.sourceFileDocument.uri
            )?.root;
            if (virtualFile instanceof vue.VueFile) {
              const isAutoImport = !!map.toSourcePosition(
                position,
                (data) =>
                  typeof data.completion === "object" &&
                  !!data.completion.autoImportOnly
              );
              if (isAutoImport) {
                const source = _context.documents.getVirtualFileByUri(
                  document.uri
                )[1];
                for (const item of result.items) {
                  ...
                  if (source && _context.typescript) {
                    if (sourceIsKebabCasing === null) {
                      const casing = await getNameCasing(
                        _context,
                        _context.typescript,
                        _context.fileNameToUri(source.fileName)
                      );
                      sourceIsKebabCasing = casing.tag === TagNameCasing.Kebab;
                    }
                    if (sourceIsKebabCasing) {
                      item.filterText = hyphenate(
      ...
      async resolveCompletionItem(item, token) {
        ...
        for (const ext of vueCompilerOptions.extensions) {
          ...
          if (
            ...
            item.additionalTextEdits[0].newText.indexOf(
              "import " + item.textEdit.newText + " from "
            ) >= 0 &&
            ((await _context.configurationHost?.getConfiguration<boolean>(
            ...
            item.additionalTextEdits[0].newText =
              item.additionalTextEdits[0].newText.replace(
                "import " + item.textEdit.newText + " from ",
                "import " + newName + " from "
              );
            ...
            new RegExp(`import \\w*${suffix}\\$1 from [\\S\\s]*`).test(
              item.textEdit.newText
            )
          ) {
        ...
        if (
          ...
          for (const [_, map] of _context.documents.getMapsByVirtualFileUri(
            ...
            if (virtualFile instanceof vue.VueFile) {
              ...
              const textDoc = _context.documents.getDocumentByFileName(
                virtualFile.snapshot,
                virtualFile.fileName
              );
              if (sfc.scriptAst && sfc.script) {
                const _scriptRanges = vue.scriptRanges.parseScriptRanges(
                  ts,
                  sfc.scriptAst,
                  !!sfc.scriptSetup,
                  true
                ...
                if (exportDefault) {
                  ...
                  if (
                    exportDefault.componentsOption &&
                    ...
                    const printText = printer.printNode(
                    ...
                    const editRange = vscode.Range.create(
                      textDoc.positionAt(
                        sfc.script.startTagEnd +
                          exportDefault.componentsOption.start
                      ),
                      textDoc.positionAt(
                        sfc.script.startTagEnd +
                          exportDefault.componentsOption.end
                      )
                    );
                    ...
                    const editRange = vscode.Range.create(
                      textDoc.positionAt(
                        sfc.script.startTagEnd + exportDefault.args.start
                      ),
                      textDoc.positionAt(
                        sfc.script.startTagEnd + exportDefault.args.end
                      )
                    );
                    autoImportPositions.add(editRange.start);
                    autoImportPositions.add(editRange.end);
      ...
      async provideCodeActions(document, range, context, token) {
        const result = await base.provideCodeActions?.(
          document,
          range,
          context,
          token
        ...
        return result?.filter(
          (codeAction) => codeAction.title.indexOf("__VLS_") === -1
        );
      },
  ...
  plugins.pug ??= createVueTemplateLanguagePlugin({
    ...
    getScanner: (document, pugPlugin): html.Scanner | undefined => {
      const pugDocument = (
        pugPlugin as ReturnType<ReturnType<typeof createPugPlugin>>
      ).getPugDocument(document);
      if (pugDocument) {
        return (pugPlugin as ReturnType<ReturnType<typeof createPugPlugin>>)
          .getPugLs()
          .createScanner(pugDocument);
      }
    },
  ...
  plugins["vue/visualizeHiddenCallbackParam"] ??=
    createVisualizeHiddenCallbackParamPlugin();
