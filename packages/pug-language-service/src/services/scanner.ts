...
export function register(htmlLs: html.LanguageService) {
	return (pugDoc: PugDocument, initialOffset = 0) => {
		...
		const htmlOffset = pugDoc.map.mappings
		...
		const scanner: html.Scanner = {
			...
			getTokenOffset: () => {
				return pugDoc.map.toSourceOffset(htmlScanner.getTokenOffset())?.[0] ?? -1;
			},
			getTokenEnd: () => {
				return pugDoc.map.toSourceOffset(htmlScanner.getTokenEnd())?.[0] ?? -1;
			},
