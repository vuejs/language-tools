...
export function register(htmlLs: html.LanguageService) {
	...
	return (pugCode: string) => {
		...
		const pugTextDoc = TextDocument.create(shared.fsPathToUri('foo.pug'), 'jade', 0, pugCode);
		...
		const pugTextDocument = TextDocument.create('foo.pug', 'jade', 0, pugCode);
		const htmlTextDocument = TextDocument.create('foo.html', 'html', 0, htmlCode);
