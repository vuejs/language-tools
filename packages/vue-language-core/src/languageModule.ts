...
export function createLanguage(
  ...
  const sharedTypesSnapshot = ts.ScriptSnapshot.fromString(
    sharedTypes.getTypesCode(vueCompilerOptions)
  );
    ...
    resolveHost(host) {
      return {
        ...
        getScriptFileNames() {
          ...
          return [...getSharedTypesFiles(fileNames), ...fileNames];
        },
        ...
        resolveModuleNameLiterals(
          ...
          return moduleLiterals.map((moduleLiteral) => {
            ...
            if (
              ...
              vueCompilerOptions.extensions.some((ext) =>
  ...
  function getSharedTypesFiles(fileNames: string[]) {
    const moduleFiles = fileNames.filter((fileName) =>
      vueCompilerOptions.extensions.some((ext) => fileName.endsWith(ext))
    );
...
export function createLanguages(
  ...
  return [
    ...
    ...(vueCompilerOptions.experimentalAdditionalLanguageModules?.map(
  ...
  try {
    ...
    if (!(ts as any).__vuePatchResolveModuleNames) {
      ...
      ts.resolveModuleName = (...args) => {
        if (
          args[6] === ts.ModuleKind.ESNext &&
          vueCompilerOptions.extensions.some((ext) => args[0].endsWith(ext))
        ) {
