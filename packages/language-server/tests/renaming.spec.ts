...
test('rename at the left boundary of a `__VLS_unwrap` read', async () => {
	// The shared source boundary between the preceding chunk and the binding
	// chunk must not resolve to disjoint generated positions (the preceding
	// chunk's end lands on the `__VLS_unwrap` helper identifier).
	expect(
		await requestRenameToTsServer(
			'tsconfigProject/fixture.vue',
			'vue',
