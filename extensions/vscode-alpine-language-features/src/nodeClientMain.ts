...
export function activate(context: vscode.ExtensionContext) {
	return commonActivate(context, async (
		...
		const maxOldSpaceSize = vscode.workspace.getConfiguration('volar').get<number | null>('alpineserver.maxOldSpaceSize');
		...
		const clientOptions: lsp.LanguageClientOptions = {
			...
			synchronize: {
				fileEvents: vscode.workspace.createFileSystemWatcher('{**/*.html,**/*.js,**/*.jsx,**/*.ts,**/*.tsx,**/*.json}')
			}
		};
